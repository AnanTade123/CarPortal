...
const SellForCarPremium = () => {
  ...
  const { data: activeCounts, refetch: activeCountRefetch } =
    useCarCountByStatusQuery({ status: active, id: id });
  const { data: pendingCounts, refetch: pendingCountRefetch } =
    useCarCountByStatusQuery({ status: pending, id: id });
  const { data: deactivateCounts, refetch: deactiveCountRefetch } =
    useCarCountByStatusQuery({ status: deactive, id: id });
  const { data: soldCounts, refetch: soldCountsRefetch } =
    useCarCountByStatusQuery({ status: sell, id: id });
  const {
    data: activeData = [],
    isLoading: isLoadingActive,
    error: errorActive,
    refetch: refetchActive,
  } = useDealerIdByCarQuery({ id, pageNo, status: active });
  const {
    data: pendingData = [],
    isLoading: isLoadingPending,
    error: errorPending,
    refetch: pendingRefeatch,
  } = useDealerIdByCarQuery({ id, pageNo, status: pending });
  const {
    data: sellData = [],
    isLoading: isLoadingSell,
    error: errorSell,
    refetch: sellRefeatch,
  } = useDealerIdByCarQuery({ id, pageNo, status: sell });
  const {
    data: deactiveData = [],
    isLoading: isLoadingDeactive,
    error: errorDeactive,
    refetch: refetchDeactive,
  } = useDealerIdByCarQuery({ id, pageNo, status: deactive });
